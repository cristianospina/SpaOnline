...
public class TipoServicioAzureSqlDAO extends SqlConnection implements TipoServicioDAO {
	
	public TipoServicioAzureSqlDAO(final Connection conexion) {
		super (conexion);
		
	}

	@Override
	public List<TipoServicioEntity> consultar(TipoServicioEntity data) {
		List<TipoServicioEntity> resultado = new ArrayList<>();
		...
		String sentenciaSql = formarSentenciaConsulta(data,parametros);
				...
				try (final PreparedStatement sentenciaPreparada = getConexion().prepareStatement(sentenciaSql)) {
			        ...
			        var mensajeUsuario = "Se ha presentado un problema intentando consultar el tipo servicio";
			        var mensajeTecnico =  "Se ha presentado una excepcion de tipo SQLException intentando consultar el tipo de servicio, por favor validar la trama completa de que sucedio...";
			        throw new DataSpaOnlineException(mensajeUsuario, mensajeTecnico, excepcion);
			    } 
				catch (final Exception excepcion) {
			        var mensajeUsuario = "Se ha presentado un problema INESPERADO,intentando consultar el tipo servicio";
			        var mensajeTecnico =  "Se ha presentado una excepcion de tipo Exception intentando consultar el tipo de servicio, por favor validar la trama completa de que sucedio...";
			        throw new DataSpaOnlineException(mensajeUsuario, mensajeTecnico, excepcion);
			    }
	...
	private final String formarSentenciaConsulta(TipoServicioEntity data , List<Object> parametros) {
		
		final StringBuilder sentenciaSql = new StringBuilder();
		String operadorCondicional ="WHERE";
		
		sentenciaSql.append("SELECT id , nombre  FROM Tiposervicio ");
